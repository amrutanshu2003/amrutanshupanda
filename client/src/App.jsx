...
const getSkillIcon = (skillName, className = "") => {
  ...
  if (skill.includes("git")) {
    return (
      <svg viewBox="0 0 24 24" className={className} aria-hidden="true" style={{ width: "16px", height: "16px", marginRight: "8px", verticalAlign: "middle", display: "inline-block" }}>
        <path fill="#F05032" d="m23.5 11.3-10.8-10.8a1.7 1.7 0 0 0-2.4 0L8.1 2.7l2.8 2.8a2.1 2.1 0 0 1 2.6 2.7l2.7 2.7a2.1 2.1 0 1 1-1.3 1.3l-2.5-2.5v6.4a2.1 2.1 0 1 1-1.6 0V9.7A2.1 2.1 0 0 1 9.7 7.6L7 4.9.5 11.3a1.7 1.7 0 0 0 0 2.4l10.8 10.8a1.7 1.7 0 0 0 2.4 0l9.8-9.8a1.7 1.7 0 0 0 0-2.4z"/>
      </svg>
    );
  }
  ...
  const matchedDeviconKey = Object.keys(deviconAliases).find((key) => {
    if (key.length <= 2) return skill === key;
    return skill.includes(key);
  });

  if (matchedDeviconKey) {
    const iconId = deviconAliases[matchedDeviconKey];
    const base = `https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${iconId}`;
    return (
      <img
        src={`${base}/${iconId}-original.svg`}
        alt=""
        className={className}
        aria-hidden="true"
        style={{ width: "18px", height: "18px", marginRight: "8px", verticalAlign: "middle", display: "inline-block" }}
